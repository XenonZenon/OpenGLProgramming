...
unsigned int Shader::compileShader(std::string type, std::string filename){
  ...
  if (type == "vertex") {
    const char * vertex = this->readShader("vertex", filename);
  ...
  if(type == "fragment"){
    const char * fragment = this->readShader("fragment", filename);
...
void Shader::setVertexShader(std::string filename){
  this->vertex = this->compileShader("vertex", filename);
}

void Shader::setFragmentShader(std::string filename){
  this->fragment = this->compileShader("fragment", filename);
}
